...
print(readLines(paste(checkout_path, "/checkout.cmd.txt", sep = "/")))

df <- read.table(paste(checkout_path, "/checkout.log.txt", sep = "/"), header=T, comment ="")
df <- df[,3:6]
...
### FigI.1 Total extraction efficiency

Overall extraction efficiency, showing the success rate of finding the **master** (main barcode containing sample-specific sequence) and **slave** (auxiallary barcode, used to protect from contamination) barcodes, as well as read overlapping (if enabled, **overlapped**).
...
### FigI.2 Extraction efficiency by sample
...
print(readLines(paste(histogram_path, "/histogram.cmd.txt", sep = "/")))
}
```

### FigII.1 UMI coverage plot

Below is the plot of MIG size distribution that was used to select MIG size threshold for assembly (shown by dashed lines, in case assembly was performed).
...
df.a <- read.table(paste(assemble_path, "/assemble.log.txt", sep = "/"), header=T, comment ="")
...
df$MIG_COUNT_THRESHOLD <- as.character(df$MIG_COUNT_THRESHOLD)
  ...
  df$MIG_COUNT_THRESHOLD <- rep("0", nrow(df))
}
...
if (!is.null(assemble_path)) {
print(readLines(paste(assemble_path, "/assemble.cmd.txt", sep = "/")))
...
### FigIII.1 Number of assembled MIGs
...
p<-plotAsm(df.1) + scale_y_continuous(expand = c(0, 0), limits=c(0,max(df.1$value)*1.2))
}
p
```

### FigIII.2 Overall assembly efficiency
...
p<-plotAsm(df.1) + scale_y_continuous(labels = percent_format(), expand = c(0, 0), limits=c(0,max(df.1$value)*1.2))
}
p
```

### FigIII.3 Fraction of assembled reads
...
### FigIII.4 Fraction of reads not fitting MIG consensus
...
print(readLines(paste(cdrblast_path, "/cdrblast.cmd.txt", sep = "/")))
...
### FigIV.1 Number of CDR3-containing MIGs
...
### FigIV.2 Number of CDR3-containing reads
...
### FigIV.4 Low-quality CDR3 filtering rate
...
print(readLines(paste(cdrfinal_path, "/cdrblastfilter.cmd.txt", sep = "/")))
...
### FigV.1 Number of clonotypes found
...
### FigV.2 Final number of TCR/Ig molecules
...
### FigV.3 Rate of hot-spot error filtering
...
### FigV.4 Fraction of non-coding clonotypes
