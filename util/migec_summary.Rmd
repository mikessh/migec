...
```{r, echo=FALSE}
plotAsm(subset(df, variable == "migs.assembled"))
```
...
```{r, echo=FALSE}
plotAsm(subset(df, variable == "umi.fraction.assembled")) + scale_y_continuous(labels = percent_format())
```
...
```{r, echo=FALSE}
plotAsm(subset(df, variable == "reads.fraction.assembled")) + scale_y_continuous(labels = percent_format())
```
...
```{r, echo=FALSE}
plotAsm(subset(df, variable == "reads.dropped.within.mig")) + scale_y_continuous(labels = percent_format())
remove(df)
```
...
plotCdr <- function(dd) {
  ggplot(dd, aes(x=sample, fill = sample, y=value)) +
    geom_bar(stat='identity', size = 1) + coord_flip() +
    scale_fill_manual(values = pal) + guides(fill = F) +
    xlab("") + ylab("") + facet_grid(.~type+metric) + 
    theme_bw()
}
```
...
```{r, echo=FALSE}
plotCdr(subset(df, variable == "final.count" & type == "asm" & metric == "mig"))
```
...
```{r, echo=FALSE}
plotCdr(subset(df, variable == "map.rate")) + scale_y_continuous(labels = percent_format())
```
...
```{r, echo=FALSE}
plotCdr(subset(df, variable == "qual.rate")) + scale_y_continuous(labels = percent_format())
```

### Step V: CdrBlastFitler

This section contains the result of hot-spot error filtering stage and final statistics
