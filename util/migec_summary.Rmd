...
Sample barcode extraction results table
...
ggplot(df.s, aes(x=sample,y=value,fill=variable)) + 
  ...
  scale_y_continuous(expand = c(0, 0)) + xlab("") + ylab("reads") +
...
ggplot(df, aes(x=sample,y=value,fill=variable)) + 
  ...
  scale_y_log10(expand = c(0, 0)) + ylab("reads") + xlab("") +
  scale_fill_brewer(palette="Paired") +
  theme_bw()
remove(df)
```
...
This section contains data on sequencing depth based on counting the number of reads tagged with the same UMI, i.e. reads that belong to the same Molecular Identifier Group (MIG). Below is the plot of MIG size distribution that was used to select MIG size threshold for assembly (shown by dashed lines).
...
plotAsm <- function(dd) {
  ggplot(dd, aes(x=sample, fill = sample, y=value)) +
    geom_bar(stat='identity', size = 1) + coord_flip() +    
...
```{r, echo=FALSE}
plotAsm(subset(df, variable == "migs.assembled")) + scale_y_continuous(expand = c(0, 0))
```
...
```{r, echo=FALSE}
plotAsm(subset(df, variable == "umi.fraction.assembled")) + scale_y_continuous(labels = percent_format(), expand = c(0, 0))
```
...
```{r, echo=FALSE}
plotAsm(subset(df, variable == "reads.fraction.assembled")) + scale_y_continuous(labels = percent_format(), expand = c(0, 0))
```
...
```{r, echo=FALSE}
plotAsm(subset(df, variable == "reads.dropped.within.mig")) + scale_y_continuous(labels = percent_format(), expand = c(0, 0))
remove(df)
```
...
```{r, echo=FALSE}
plotCdr(subset(df, variable == "final.count" & type == "asm" & metric == "mig")) + scale_y_continuous(expand = c(0, 0))
```
...
```{r, echo=FALSE}
plotCdr(subset(df, variable == "map.rate")) + scale_y_continuous(labels = percent_format(), expand = c(0, 0)) + facet_grid(.~type+metric)
```
...
```{r, echo=FALSE}
plotCdr(subset(df, variable == "qual.rate")) + scale_y_continuous(labels = percent_format(), expand = c(0, 0)) + facet_grid(.~type+metric)
remove(df.r)
remove(df.m)
remove(df)
```
...
df <- data.frame(sample = df$X.SAMPLE_ID, 
                 clones.count = df$CLONOTYPES_TOTAL - df$CLONOTYPES_FILTERED,
                 migs.count = df$EVENTS_TOTAL - df$EVENTS_FILTERED,
...
plotCdrFinal.2 <- function(dd) {
  ggplot(dd, aes(x=sample, fill = sample, y=value)) +
    ...
    theme_bw() + scale_y_continuous(labels = percent_format(), expand = c(0, 0))
}
```
